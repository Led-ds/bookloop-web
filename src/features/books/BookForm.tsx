...
export interface BookFormProps {
  ...
  onServerErrorsClear?: (key: string) => void;
}

/** Formulário compartilhado por cadastro e edição — mesma validação, sem duplicar lógica. */
export function BookForm({
  ...
  const [form, setForm] = useState<BookInput>(initialValues);
  ...
  const set = (k: keyof BookInput, v: string | boolean) => {
    setForm((f) => ({ ...f, [k]: v }));
    onServerErrorsClear?.(k as string);
  };
